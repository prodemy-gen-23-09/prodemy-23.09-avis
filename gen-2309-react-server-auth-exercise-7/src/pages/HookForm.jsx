  ...
  const onSubmitForm = (data) => {
    axios.get('http://localhost:8000/wishlist')
      .then(wishlistResponse => {
        const productData = wishlistResponse.data.flatMap(wishlist => wishlist.products);
        const totalPrice = productData.reduce((total, product) => total + product.price, 0);
  
        axios.get('http://localhost:8000/users')
          .then(userResponse => {
            const user = userResponse.data.find(user => user.email === data.email);
            ...
            axios.post("http://localhost:8000/booking", payload)
              .then(() => {
      ...
      .catch(productsError => console.error("Error fetching product data:", productsError));
  };
